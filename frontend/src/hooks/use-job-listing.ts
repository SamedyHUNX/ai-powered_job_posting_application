...
export function useJobListing() {
    ...
    const fetchJobListingsByOrganization = useCallback(async (organizationId: string) => {
        try {
            dispatch(setLoading(true));
            const response = await jobListingsApi.findByOrganization(organizationId);
            dispatch(
                setJobListings({
                    jobListings: response.jobListings,
                    count: response.count,
    ...
    const createJobListingMutation = useMutation({
        mutationFn: (dto: CreateJobListingDto) => {
